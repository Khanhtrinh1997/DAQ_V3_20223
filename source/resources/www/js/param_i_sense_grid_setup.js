...
function PMStatusGet()
{
    ...
    if(powermeter)
    {
        powermeter.open("GET", "i_sense_status?=", true);
...
function fPMID1Set () {	
	...
	if(vPMVD1Set)	{
		if(vPMVD1DataSet.value != ""){
			vPMVD1Set.open("GET", "/i_sensenum1v_value=" + vPMVD1DataSet.value + "&id=" + Math.random(), true);
			vPMVD1Set.send(null);
		}
	}
}
...
function fPMTotalNumSet (){
	...
	if(vPMTotalNumSet)	{
		if(vPMTotalNumDataSet.value != ""){
			vPMTotalNumSet.open("GET", "/i_senseTotal_value=" + vPMTotalNumDataSet.value + "&id=" + Math.random(), true);
